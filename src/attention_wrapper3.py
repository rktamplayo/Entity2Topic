...
def _bahdanau_score(processed_query, keys, normalize, processed_context):
  ...
  processed_context = array_ops.expand_dims(processed_context, 1)
  ...
  if normalize:
    ...
    return math_ops.reduce_sum(
        normed_v * math_ops.tanh(keys + processed_query + processed_context + b), [2])
  else:
    return math_ops.reduce_sum(v * math_ops.tanh(keys + processed_query + processed_context), [2])
...
class BahdanauAttention(_BaseAttentionMechanism):
  ...
  def __init__(self,
    ...
    super(BahdanauAttention, self).__init__(
        ...
        name=name)
    self.context_layer = layers_core.Dense(num_units, name="context_layer", use_bias=False)
    self._num_units = num_units
    self._normalize = normalize
    self._name = name

  def __call__(self, query, previous_alignments, context):
    ...
    with variable_scope.variable_scope(None, "bahdanau_attention", [query]):
      ...
      processed_context = self.context_layer(context) if self.context_layer else context
      score = _bahdanau_score(processed_query, self._keys, self._normalize, processed_context)
...
def _compute_attention(attention_mechanism, cell_output, previous_alignments,
                       attention_layer, cell_context):
  """Computes the attention and alignments for a given attention_mechanism."""
  alignments = attention_mechanism(
      cell_output, previous_alignments=previous_alignments, context=cell_context)
  ...
  def call(self, inputs, state):
    ...
    for i, attention_mechanism in enumerate(self._attention_mechanisms):
      attention, alignments = _compute_attention(
          ...
          self._attention_layers[i] if self._attention_layers else None, self.context)
